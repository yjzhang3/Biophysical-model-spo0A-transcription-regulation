function [diff,sim_data] = fit_Hill_diffvmax(p,real_data)

K_Pnotspo0A = p(1); % 1/exp(-G_promoter)

vmax_Pnotspo0A = p(2); % maximum initiation rate
vmax_Pspo0A = p(3);
...
%     sim_Pspo0A(i) = vmax_Pspo0A*R_time(i)/(R_time(i) + K_Pspo0A);
    sim_Pspo0A(i) = vmax_Pspo0A*R_time(i)/(R_time(i) + K_Pnotspo0A);
end
