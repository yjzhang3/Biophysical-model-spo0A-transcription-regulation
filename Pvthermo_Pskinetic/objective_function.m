function [final_diff,sim_data_Ps,sim_data_Pv,sim_data_PsPv,...
    ...
    H_conc_t,A_conc_t,energyi_s,energyi_v,energyi_sv] = objective_function_noquaE(...
    nbd,p,TF_conc_t,...
...
% 23sv (30)
% without constraint
% ne = nbd+nchoosek(nbd,2)+nchoosek(nbd,3);
% energyi_sv = p(1:ne);
...
%% non-multiplicative vmax for Ps
% ps: v1,v2,v3,v13,v23,v123,v000 (kinetic)
% pv: one vmax for all (thermo)

% load("vmax_rnap_Hill_constraint.mat")
fng_s = fieldnames(group_array_s);
fng_v = fieldnames(group_array_v);
fng_sv = fieldnames(group_array_sv);
ns = 8; % vmax000 is  known
nv = numel(fng_v); %1
ntot = ns+nv;
vmax_raw = p(ne+1:ne+ntot);

vmax_s_raw = vmax_raw(1:ns); % stores the optimized vmax parameter for Ps 
vmax_v_final = vmax_raw(ns+1:ntot); % plug in Pv vmax from the fit to 123*

% 000,001,010,011,100,101,110,111
% v1,v2,v3,v13,v23,v123
vmax_s_final = [vmax_s_raw(8),vmax_s_raw(3),vmax_s_raw(2),vmax_s_raw(5),...
    vmax_s_raw(1),vmax_s_raw(4),vmax_s_raw(7),vmax_s_raw(6)];

vmax_sv_final = zeros(1,length(fng_s));

% every configuration has additive vamx from both ps and pv
for i = 1:length(fng_s)
    vmax_sv_final(i) = vmax_v_final(1)+vmax_s_final(i);
end
